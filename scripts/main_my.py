...
class Mission:
    def __init__(self):
        ...
        self.local_pose_stamped_pub = rospy.Publisher('/move_base_simple/goal', PoseStamped, queue_size=10)
    ...
    def setWayPoints(self):
        ...
        # self.wpTakeoff = Point(0, 0, rospy.get_param('/takeoff_alt', 3.0))
    ...
    def setMyWayPoints(self):
        
        self.wp1 = self.ConvertLocalPoseStamped(rospy.get_param('/waypoint/wp1')) 
        self.wp2 = self.ConvertLocalPoseStamped(rospy.get_param('/waypoint/wp2')) 
        self.wp3 = self.ConvertLocalPoseStamped(rospy.get_param('/waypoint/wp3')) 
    ...
    def PubLocalPosition(self, point):
        ...
        self.local_pose_stamped_pub.publish(pose)
    ...
    def ConvertLocalPoseStamped(self, global_point):
        local_pose_stamped = PoseStamped()

        home_lat = self.global_home_position.latitude
        home_lon = self.global_home_position.longitude

        local_pose_stamped.pose.position.x = self.earth_radius * cos(home_lat * pi / 180) * (global_point[1] - home_lon) * pi / 180
        local_pose_stamped.pose.position.y = self.earth_radius * (global_point[0] - home_lat) * pi / 180
        local_pose_stamped.pose.position.z = global_point[2]

        return local_pose_stamped
    ...
    def processsing(self):
        process = {0:['WP1', self.wp1],
                   1:['WP2', self.wp2],
                   2:['WP3', self.wp3],
                   3:['Land', self.local_home_position]}.get(self.step, 'END')

        if (process[0] == 'WP1') or (process[0] == 'WP2') or (process[0] == 'WP3'):
        ...
        #self.PublishWayPoint(self.step)
        result = self.waypoint_reach_check(process, 1.5)
